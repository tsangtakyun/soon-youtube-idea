  ...
  return [
    {
      ...
      backingInfoNeeded: [
        'What most outsiders get wrong about ' + topic,
        'The real cultural or economic driver behind it',
        'A specific example that makes it tangible',
      ],
    ...
    },
    {
      ...
      backingInfoNeeded: [
        'A specific wealthy example that feels absurd or aspirational',
        'The contrast from the poor side of the same story',
        'Data or stat that shows the scale of the gap',
      ],
    ...
    },
    {
      ...
      backingInfoNeeded: [
        'The most common Western misconception about this topic',
        'What actually drives the situation from the inside',
        'Historical or cultural context that changes the reading',
      ],
    ...
    },
    {
      ...
      backingInfoNeeded: [
        'Specific country or city in Southeast Asia where this plays out',
        'What makes this version different from the East Asia story',
        'Human detail or anecdote that makes it real',
      ],
    ...
    },
    {
      ...
      backingInfoNeeded: [
        'The specific recent development or shift',
        'Why this matters beyond what the headline says',
        'What happens next — the implication most people are missing',
      ],
...
async function buildClaudeIdeas(
  ...
  const refLines = rows.map((row, index) =>
    (index + 1) + '. ' + row.title + ' | ' + row.channel + ' | ' + (row.views ?? 0) + ' views | ' + row.url
  ).join('\n')
  ...
  const prompt = [
    ...
    '- Based in Hong Kong, explaining Asia absurdities, contradictions, and realities in English',
    '- Not a travel channel — cultural and social analysis',
    '- Audience: English-speaking viewers curious about Asia who are not locals',
    '- Format: 12-20 minute deep analysis, strong framework plus real credibility',
    '- Key advantage: genuine Asian insider perspective that Western creators cannot replicate',
    '',
    'Task: Based on the input and reference data, output 5 idea cards.',
    'Each card must have a category, choose one from:',
    '- breaking (current affairs, trending news angle)',
    '- culture (cultural phenomena, social behavior)',
    '- rich (wealth, luxury, affluent Asia)',
    '- poor (poverty, survival, developing Asia)',
    '- evergreen (timeless deep analysis)',
    '',
    'Each card output: title, category, coreAngle, whyNow, audienceFit, breakoutPattern,',
    'backingInfoNeeded (array of 3 strings), seriesExtensions (array of 3 strings), references (array of URLs)',
    '',
    'Write all card content in English.',
    'Titles should be punchy and hook-driven, suitable for YouTube thumbnails.',
    '',
...
async function buildClaudeIdeas(payload: YoutubeIdeaSearchPayload, rows: AlgrowVideo[]) {
  ...
  const anthropic = new Anthropic({ apiKey })
  const refLines = rows.map((row, i) => (i + 1) + '. ' + row.title + ' | ' + row.channel + ' | ' + (row.views ?? 0) + ' views | ' + row.url).join('\n')
  const prompt = [
    'You are the AI assistant for SOON YouTube Idea Radar.',
    '',
    'SOON positioning: The Insider Asia',
