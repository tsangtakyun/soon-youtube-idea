...
async function buildClaudeIdeas(payload: YoutubeIdeaSearchPayload, rows: AlgrowVideo[]) {
  ...
  const prompt = [
  '你係 SOON YouTube Idea Radar 嘅 AI 助手。',
  '',
  'SOON 嘅定位：The Insider\'s Asia',
  '— 由香港出發，用英文解釋亞洲嘅荒誕、矛盾、同真實',
  '— 唔係旅行頻道，係文化同社會分析',
  '— 受眾：英語觀眾想理解亞洲，有好奇心但唔係當地人',
  '— 格式：12-20分鐘深度分析，強烈框架 + 真實可信度',
  '',
  '任務：根據輸入同參考數據，輸出 5 張題材卡。',
  '每張卡必須有 category，從以下選一：',
  '- "breaking" (時事熱話)',
  '- "culture" (文化現象、社會行為)',
  '- "rich" (財富、奢靡、富裕亞洲)',
  '- "poor" (貧窮、求存、發展中亞洲)',
  '- "evergreen" (唔過時的深度分析)',
  '',
  '每張卡輸出：title, category, coreAngle, whyNow, audienceFit, breakoutPattern,',
  'backingInfoNeeded (array of 3), seriesExtensions (array of 3), references (array of URLs)',
  '',
  `輸入模式：${payload.mode}`,
  `輸入內容：${payload.query}`,
  `語言：${payload.language}`,
