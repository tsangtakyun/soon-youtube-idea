      ...
      <div style={{ maxWidth: '1180px', margin: '0 auto', display: 'grid', gap: '20px' }}>
        ...
        >
          ...
          >
            ...
            >
              用關鍵字、頻道 URL 或影片 URL，先由 YouTube 爆款 pattern 倒推出 5 張值得拍的題材卡。這一步先做 research 同 angle planning，唔係直接寫死 script。
            </p>
          ...
          >
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              {[
                'YouTube 題材研究不應一開始便直接寫死 hook，先與腳本系統分開會更清晰。',
                '先找出值得製作的題材方向，再決定補充資料、系列延伸與腳本規劃。',
                '將單條爆款模式拆成可複製的內容系列，才是 YouTube 最有價值的部分。',
              ].map((item) => (
                <div
                  key={item}
                  style={{
        ...
        <section
          style={{
            display: 'grid',
            gridTemplateColumns: 'minmax(0, 0.94fr) 1.06fr',
            gap: '20px',
            alignItems: 'start',
          }}
        >
          ...
          >
            <div style={{ fontSize: '12px', letterSpacing: '0.16em', color: '#8b7c69' }}>
              INPUT
            </div>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                width: 'fit-content',
                opacity: isPending || !query.trim() ? 0.72 : 1,
              }}
            >
            ...
            {error ? (
              ...
              >
                如 Algrow endpoint 尚未完成設定，系統會先以 sample reference rows 與 fallback 流程生成題材，方便你先搭建完整工作流程。
              </div>
            )}
          </section>
          ...
          <section style={{ display: 'grid', gap: '16px' }}>
            ...
            >
              ...
              {result?.algrowRows?.length ? (
                  ...
                  {result.algrowRows.map((row, index) => (
                    ...
                    >
                      ...
                      <div style={{ color: '#7b6d57', lineHeight: 1.7 }}>
                        {String(row.channel ?? '')} · {String(row.views ?? '')} views ·{' '}
                ...
                <div style={{ color: '#7b6d57', lineHeight: 1.7 }}>
                  生成後，這裡會顯示 Algrow 或 sample 參考資料。
                </div>
              )}
            </section>
            ...
            {result ? (
              ...
              >
                ...
                <div style={{ display: 'grid', gap: '12px' }}>
                  ...
                  >
                    已生成 5 張 YouTube 題材卡，可開始進行內部審視、系列延伸討論，再帶入腳本規劃。
                  </div>
                  <div
                    style={{
                      padding: '14px 16px',
                  ...
                  >
                    {result.saveStatus === 'saved'
                      ...
                      : result.saveStatus === 'failed'
                        ...
                        : '目前尚未設定 server-side Supabase，這次只完成生成。'}
                  </div>
                </div>
              </section>
            ) : null}
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                {[
                  ['核心角度', idea.coreAngle],
                  ['值得切入的原因', idea.whyNow],
                  ['受眾適配度', idea.audienceFit],
                  ['爆款模式', idea.breakoutPattern],
                  ['需要補充的資料', idea.backingInfoNeeded.join(' / ')],
